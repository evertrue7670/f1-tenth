    ...
    def calc_arclength(
        ...
        def distance_to_spline(s):
            try:
                ...
                dist_sq = (x - x_eval) ** 2 + (y - y_eval) ** 2
                dist_sq = max(0.0, dist_sq)  # Ensure non-negative
        ...
        try:
            ...
            absolute_distance = float(output[1])
            
            # Validate results
            if np.isnan(closest_s) or np.isinf(closest_s) or \
               np.isnan(absolute_distance) or np.isinf(absolute_distance):
                # Fallback to inaccurate method
                return self.calc_arclength_inaccurate(x, y)
            
            return closest_s, absolute_distance
        except:
            # Fallback to inaccurate method on any error
            return self.calc_arclength_inaccurate(x, y)
